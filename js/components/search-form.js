class SearchForm extends HTMLElement {
  ...
  connectedCallback() {
    this.shadow.innerHTML = `
      <style>
        @import 'css/components/search-form.css';
      </style>
      <div class="form-container">
        ...
        <form novalidate="novalidate" onsubmit="return false;" class="searchbox sbx-custom">
          <div role="search" class="sbx-custom__wrapper">
            <input autofocus="true"  type="search" name="search" placeholder="Search movie" autocomplete="off" required="required" class="sbx-custom__input">
  ...
  handleReset() {
    const searchBox = this.shadow.querySelector('input');
    this.shadow.querySelector('.searchbox [type="reset"]').addEventListener('click', () => {
      searchBox.focus();
    });
  }
  ...
  search(s){
    const event = new CustomEvent('search-api', {
      detail: {s}
    });
  ...
  removeMessage() {
    const event = new CustomEvent('search-message-reset');
    window.dispatchEvent(event);
  }

  handleSubmit(){
    const searchBox = this.shadow.querySelector('input');
    this.shadow.querySelector('form').addEventListener('submit', () => {
      if(searchBox.value === '') {
        this.displayMessage('Please insert your query');
        searchBox.focus();
      } else {
        this.removeMessage();
